...
public static class SlashCommandCatalog
{
    ...
    private static readonly List<SlashCommand> ClaudeBuiltins = new()
    {
        ...
        new("/code-review", "Review the current diff, a pull request, or a path for correctness bugs.", "現在の差分や Pull Request、パスのコードをレビューし、不具合を検出します。"),
        new("/simplify", "Suggest simplifications and cleanups for the current diff.", "現在の差分に対する簡潔化・整理の提案を行います。"),
        new("/security-review", "Review the current diff for security vulnerabilities.", "現在の差分にセキュリティ上の脆弱性がないかレビューします。"),
        new("/verify", "Generate and run tests to verify the current diff or PR.", "現在の差分や PR に対してテストを生成・実行して検証します。"),
        new("/agents", "Manage the subagents available to this session.", "このセッションで使えるサブエージェントを管理します。"),
        new("/plan", "Enter plan mode to design an approach before making changes.", "変更を加える前にプランモードへ入り、実装方針を設計します。"),
        new("/effort", "Set the reasoning effort level (low/medium/high/xhigh/max).", "推論のエフォートレベル（low/medium/high/xhigh/max）を設定します。"),
        ...
        new("/diff", "Review the changes in the working tree.", "作業ツリーの変更内容をレビューします。"),
        new("/cd", "Move the session to a new working directory.", "セッションの作業ディレクトリを変更します。", NeedsArgument: true),
        new("/todos", "List, complete, or create todos for this session.", "このセッションの Todo を一覧・完了・作成します。"),
        ...
        new("/output-style", "Switch the output style used for responses.", "応答に使用する出力スタイルを切り替えます。"),
        new("/ide", "Manage IDE integration and show its status.", "IDE 連携を管理し、状態を表示します。"),
        ...
        new("/release-notes", "Show the latest Claude Code release notes.", "最新の Claude Code リリースノートを表示します。"),
        new("/upgrade", "Show available Claude Code version upgrades.", "利用可能な Claude Code のアップグレードを表示します。"),
        new("/bug", "Report a bug or share this session with Anthropic.", "バグ報告やこのセッションの共有を行います。"),
        new("/privacy-settings", "Manage what data Claude Code saves and sends.", "Claude Code が保存・送信するデータを管理します。"),
    };
